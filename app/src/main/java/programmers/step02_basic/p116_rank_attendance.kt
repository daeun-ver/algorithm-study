...
class p116_rank_attendance {
    fun solution(rank: IntArray, attendance: BooleanArray): Int {
        var answer: Int = 0
        var attendance_true = mutableListOf<Int>()
        for (i in 1 until rank.size + 1) {
            if (attendance[rank.indexOf(i)]) {
                attendance_true += rank.indexOf(i)
            }
        }
        answer = attendance_true[0] * 10000 + attendance_true[1] * 100 + attendance_true[2]
        return answer
    }
}